...
const Portfolio = (() => {
    ...
    const initCertificationsSlider = () => {
        ...
        const showCertification = (index) => {
            ...
            state.currentCertification = index;
        };

        const nextCertification = () => {
            const nextIndex = (state.currentCertification + 1) % certSlides.length;
            showCertification(nextIndex);
        };

        const prevCertification = () => {
            const prevIndex = (state.currentCertification - 1 + certSlides.length) % certSlides.length;
            showCertification(prevIndex);
        };
        ...
        dots.forEach((dot, index) => {
            dot.addEventListener('click', () => showCertification(index));
        });
